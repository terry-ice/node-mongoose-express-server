/*
 * @Author: terry
 * @Date: 2019-05-12 19:38:00
 * @Last Modified by: https://github.com/terry-ice
 * @Last Modified time: 2019-05-12 20:40:54
 */
...
authCtrl.POST = ({ body: { password } }, res) => {
  Auth.find({}, '-_id password')
    .then(
      ([
        auth = { password: md5Decode(config.AUTH.defaultPassword) },
      ]) => {
...
authCtrl.PUT = ({ body: auth }, res) => {
  ...
  if (
    ...
    (!new_password ||
      !rel_new_password ||
      !Object.is(new_password, rel_new_password))
  ) {
  ...
  if (
    !!password &&
    [new_password, rel_new_password].includes(password)
  ) {
  ...
  Auth.find({}, '_id name slogan gravatar password')
    .then(
      ([
        _auth = { password: md5Decode(config.AUTH.defaultPassword) },
      ]) => {
        if (
          !!password &&
          !Object.is(_auth.password, md5Decode(password))
        ) {
          handleError({ res, message: '原密码不正确' });
        } else {
          if (!auth.password) delete auth.password;
          if (auth.rel_new_password)
...
export default (req, res) => {
  handleRequest({ req, res, controller: authCtrl });
};
