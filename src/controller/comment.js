/*
 * @Author: terry
 * @Date: 2019-05-12 21:10:07
 * @Last Modified by: https://github.com/terry-ice
 * @Last Modified time: 2019-05-12 22:44:30
 */
/*
 *
 * 评论控制器
...
import { akismetClient } from '../utils/akismet';
...
const sendMailToAdminAndTargetUser = (comment, permalink) => {
  ...
  sendMail({
    to: 'terryloveyan@gmail.com',
    subject: '博客有新的留言',
    text: `来自 ${comment.author.name} 的留言：${comment.content}`,
    html: `<p> 来自 ${
      comment.author.name
    } 的留言：${commentContent}</p><br><a href="${permalink}" target="_blank">[ 点击查看 ]</a>`,
  });
  if (!!comment.pid) {
    Comment.findOne({ id: comment.pid }).then(parentComment => {
      sendMail({
        to: parentComment.author.email,
        subject: '你在592php.me有新的评论回复',
        text: `来自 ${comment.author.name} 的评论回复：${
          comment.content
        }`,
        html: `<p> 来自${
          comment.author.name
        } 的评论回复：${commentContent}</p><br><a href="${permalink}" target="_blank">[ 点击查看 ]</a>`,
      });
    });
  }
};
...
commentCtrl.list.POST = (req, res) => {
  ...
  const doSaveComment = () => {
    ...
    // 永久链接
    const permalink =
      'https://blog.592php.com/' +
      (Object.is(comment.post_id, 0)
        ? 'guestbook'
    ...
    //     if (
    //       ips.includes(comment.ip) ||
    //       mails.includes(comment.author.email) ||
    //       (keywords.length &&
    //         eval(`/${keywords.join('|')}/ig`).test(comment.content))
    //     ) {
    //       handleError({
    //         res,
    //         err: '内容||ip||邮箱 => 不合法',
...
commentCtrl.list.PATCH = (
  ...
  if (
    !comments ||
    !comments.length ||
    Object.is(state, null) ||
    Object.is(state, NaN) ||
    ![-1, -2, 0, 1].includes(state)
  ) {
...
// 修改单个评论
commentCtrl.item.PUT = (
  { params: { comment_id }, body: comment },
  res,
) => {
...
exports.list = (req, res) => {
  handleRequest({ req, res, controller: commentCtrl.list });
};
exports.item = (req, res) => {
  handleRequest({ req, res, controller: commentCtrl.item });
};
